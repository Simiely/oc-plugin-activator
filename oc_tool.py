...
# 修正：PyInstaller 打包后，用 sys.executable 获取 exe 真实路径
...
def load_config():
    if os.path.exists(CONFIG_FILE):
        ...
        with open(CONFIG_FILE, "r", encoding="utf-8") as f:
            return json.load(f)
    return {"username": "", "c4doctane_target": "", "octane_target": ""}
...
class App:
    ...
    def setup_ui(self):
        ...
        # c4doctane 目标路径
        ttk.Label(cfg_frame, text="c4doctane 复制目标：").grid(row=1, column=0, sticky="w", pady=6)
        self.c4d_var = tk.StringVar(value=self.config.get("c4doctane_target", ""))
        ttk.Entry(cfg_frame, textvariable=self.c4d_var, width=28).grid(row=1, column=1, sticky="ew", padx=(5, 5), pady=6)
        ttk.Button(cfg_frame, text="选择", command=self.on_select_c4d,
                    padding=(8, 2)).grid(row=1, column=2, pady=6)

        # octane 目标路径
        ttk.Label(cfg_frame, text="octane 复制目标：").grid(row=2, column=0, sticky="w", pady=6)
        self.oct_var = tk.StringVar(value=self.config.get("octane_target", ""))
        ttk.Entry(cfg_frame, textvariable=self.oct_var, width=28).grid(row=2, column=1, sticky="ew", padx=(5, 5), pady=6)
        ttk.Button(cfg_frame, text="选择", command=self.on_select_oct,
                    padding=(8, 2)).grid(row=2, column=2, pady=6)

        # 保存按钮
        ttk.Button(cfg_frame, text="保存配置", command=self.on_save_config,
                    padding=(10, 3)).grid(row=3, column=0, columnspan=3, pady=(6, 0))
        ...
        self.c4d_var.trace_add("write", lambda *a: self.refresh_preview())
        self.oct_var.trace_add("write", lambda *a: self.refresh_preview())

        # ===== 功能按钮 =====
        btn_frame = ttk.LabelFrame(self.root, text=" 功能操作 ", padding=16)
        btn_frame.pack(fill="x", padx=20, pady=(0, 10))
        btn_inner = ttk.Frame(btn_frame)
        btn_inner.pack(expand=True)

        btn_style = {"width": 40, "padding": (0, 7)}

        ttk.Button(btn_inner, text="🗑 清空 OctaneRender (Local)",
                    command=self.on_clean_local, **btn_style).pack(pady=4, fill="x")
        ttk.Button(btn_inner, text="🗑 清空 OctaneRender (Roaming)",
                    command=self.on_clean_roaming, **btn_style).pack(pady=4, fill="x")
        ttk.Button(btn_inner, text="📋 复制 AppData 到用户目录",
                    command=self.on_copy_appdata, **btn_style).pack(pady=4, fill="x")
        ttk.Button(btn_inner, text="📋 复制 c4doctane 到目标路径",
                    command=self.on_copy_c4doctane, **btn_style).pack(pady=4, fill="x")
        ttk.Button(btn_inner, text="📋 复制 octane 到目标路径",
                    command=self.on_copy_octane, **btn_style).pack(pady=4, fill="x")
        ...
        # 底部显示 exe 实际路径
        info = ttk.Label(self.root, text=f"程序目录：{APP_DIR}",
                         font=("Microsoft YaHei", 8), foreground="gray")
        info.pack(pady=(0, 8))

    def refresh_preview(self):
        username = self.username_var.get().strip()
        c4d_target = self.c4d_var.get().strip()
        ...
        oct_src = os.path.join(APP_DIR, "octane")

        text = (
            f"清空 Local  ：{local_path}\n"
            f"清空 Roaming：{roam_path}\n"
            f"复制 AppData ：{appdata_src}  →  C:\\Users\\{username if username else '(请填写用户名)'}\\AppData\n"
            f"复制 c4doctane：{c4d_src}  →  {c4d_target if c4d_target else '(请填写目标路径)'}\n"
            f"复制 octane  ：{oct_src}  →  {oct_target if oct_target else '(请填写目标路径)'}"
        )
    ...
    def on_copy_octane(self):
        username = self.get_username()
        if not username:
            return
        target = self.oct_var.get().strip()
        if not target:
            messagebox.showerror("错误", "请先填写或选择 octane 复制目标路径！")
            return
        src = os.path.join(APP_DIR, "octane")
        if not os.path.exists(src):
            messagebox.showerror("错误", f"源文件夹不存在：\n{src}")
            return
        if not messagebox.askyesno("确认复制", f"确定要复制吗？\n\n源：{src}\n目标：{target}"):
            return
        self.log(f"[复制 octane] 开始：{src} → {target}")
        if safe_copy_folder(src, os.path.join(target, "octane"), self.log):
            self.log("✅ 完成\n")
            messagebox.showinfo("完成", "octane 复制完成！")
        else:
            self.log("❌ 失败\n")
